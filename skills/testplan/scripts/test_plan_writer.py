...
class TestHeaderIsTheNewPlans(unittest.TestCase):
    """A regenerated plan's header is emitted exactly as the new plan wrote it: nothing
    from the previous plan's header is added, and the report carries only the id lists."""

    def test_a_regenerated_plans_header_is_taken_from_the_new_plan_as_is(self):
        previous = PREVIOUS.replace("## Cases", "scope: e2e\n\n## Cases", 1)
        merged, report = merge_allure_ids(previous, REGENERATED)
        prologue = lambda text: text.split("## Cases", 1)[0]
        self.assertEqual(prologue(merged), prologue(REGENERATED))
        self.assertNotIn("scope", parse_header(merged))
        self.assertEqual(sorted(report),
                         ["carried", "orphaned", "retained", "suspect", "unmatched"])
