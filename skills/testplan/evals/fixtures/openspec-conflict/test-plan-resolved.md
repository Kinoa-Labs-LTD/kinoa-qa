...
- type: negative
- priority: P1
- purpose: Verify that a correct password is still refused while the admin account is locked.
- source: ac: AC-2
- preconditions: The admin account is LOCKED after 5 failed attempts.
  No operator unlock has been performed.
- steps:
  1. Sign in with the CORRECT password while the account is LOCKED.
     → expected: The sign-in is refused with HTTP 423 and no session is granted.
  2. Read the account state.
     → expected: The account state is still LOCKED.
